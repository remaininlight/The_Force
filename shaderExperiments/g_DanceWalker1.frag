...
float walkingDude(vec2 uv, float time, vec3 groove){
...
uv.x=mod(uv.x,3.0)-1.5;
	...
	vec2 bodyPos=vec2(0.0,groove.x*0.03+0.5-floorHeight);
	...
	vec2 footLeft=footDist+vec2(groove.y*0.05, groove.x*0.1)*0.4;
	//float footLeftRadius=cos(time*4.0+PI)*0.05+0.15;
	float footLeftRadius=0.1+groove.x*-0.01;
    ...
    vec2 footRightPos=footDist+vec2(groove.y*-0.05, groove.x*-0.1)*0.4;

	val+=circleHalve(uv+footRightPos,0.1+groove.x*0.01);
...
float walkingDudeScaled(vec2 uv,float time,float scale, vec3 groove){

	return walkingDude(uv*scale+vec2(0,scale),time, groove);
}
void main()
{
    ...
    st.x+=time;
    //body
    val=walkingDudeScaled(st, time, 1.0, groove);
    st.x = uvorig.x + time * .5;
    val = max(val,walkingDudeScaled(st+vec2(-1.0,0.0),345.0+time*1.0,2.0, groove)*0.5);
    //val+=walkingDudeScaled(uv+vec2(1,0),2314+iTime*1,2)*0.75;
    st.x=uvorig.x+time*.25;

    val=max(val,walkingDudeScaled(st+vec2(1.5,0.0),34256.0+time*1.0,4.0, groove)*0.25);
